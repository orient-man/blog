...
Then: [![chevron](/images/posts/connect-the-dots/chevron.png)](/images/posts/connect-the-dots/chevron.png) Yay! Now every programmer can be an icon designer ...and it makes for decent F# kata.
...
Our goal is to implement _ParserApi_ function transforming ASCII representation into list of shapes. We can start by extracting dots with their positions (_string \[\] -> ((Dot \* Opacity) \* int) list_):
...
That's all - parser is ready. What's left is to generate bitmap from list of shapes. But drawing is boring: mostly an API driven code. I'll show you only the type signature (the rest is on [GitHub](https://github.com/orient-man/AsciiImageFsharp/blob/master/DrawingImplementation.fs)):

```fsharp
type DrawingApi = Parser.ParserApi -> int -> string [] -> System.Drawing.Bitmap
```

Given a parser, scale and ASCII representation, _DrawingApi_ implementation should return a bitmap. Drawing module depends only on a parser abstraction. In the composition root (aka main) you tie it together:
